...
const Dashboard: React.FC = () => {
  ...
  const isComplete = (r: DataRecord) => {
    ...
    return check(r.state_name) && check(r.state_code) && check(r.sch_num) && check(r.sch_name) && check(r.cust_name) && check(r.cust_code) && check(r.cust_town) && check(r.lga) && check(r.type) && check(r.category) && check(r.accd_year) && check(r.locality);
  };
